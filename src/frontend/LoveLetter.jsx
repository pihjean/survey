...
const PARA_SPEED       = 36;  // ms per char for everything else

const INITIAL_WAIT   = 5000; // silence before "Mahal," starts
const AFTER_MAHAL    = 2000; // pause after "Mahal," before first paragraph
const BETWEEN_BLOCKS = 3000; // pause between all other blocks
...
export default function LoveLetter() {
  ...
  // Render state — array of { type, text, display }
  const [blocks, setBlocks]         = useState([]);
  const [activeBlock, setActiveBlock] = useState(-1); // which block shows cursor
  ...
  // Refs to hold mutable loop state — avoids stale closures on mobile
  const timerRef    = useRef(null);
  const blockIdx    = useRef(0);
  ...
  // ── Master typewriter loop — runs entirely via refs, no stale closures ──
  const tick = useCallback(() => {
    if (isPausing.current) return;

    const bi = blockIdx.current;
    if (bi >= FULL_TEXT.length) {
      setTypingDone(true);
      setActiveBlock(-1);
      return;
    }

    const block = FULL_TEXT[bi];
    const ci    = charIdx.current;

    if (ci < block.text.length) {
      // Type one character
      const display = block.text.slice(0, ci + 1);
      setBlocks((prev) => {
        const next = [...prev];
        next[bi] = { type: block.type, text: block.text, display };
        return next;
      });
      ...
      setActiveBlock(-1); // hide cursor during pause

      const pause = bi === 0 ? AFTER_MAHAL : BETWEEN_BLOCKS;
      timerRef.current = setTimeout(() => {
        isPausing.current  = false;
        blockIdx.current   = bi + 1;
        charIdx.current    = 0;
        setActiveBlock(bi + 1);
        tick();
      }, pause);
  ...
  // ── Start typewriter after letter is visible ──
  useEffect(() => {
    ...
    // Initial 5-second wait before first character
    timerRef.current = setTimeout(tick, INITIAL_WAIT);

    return () => { if (timerRef.current) clearTimeout(timerRef.current); };
  }, [phase, letterVisible, tick]);
  ...
  const handleSubmit = () => {
    if (inputName.trim().toLowerCase() === CORRECT_NAME.toLowerCase()) {
      setError("");
      setPhase("loading");
    } else {
        ...
        <div className="letter-container">
          ...
          <div className="letter-body">
            {blocks.map((block, i) => {
              ...
              // Show cursor only on the active block while not pausing, and never on salutation after it finishes
              const showCursor = i === activeBlock && !typingDone && i !== 0;
              // Salutation: cursor only while it is actively typing (activeBlock===0)
              const showSalutationCursor = i === 0 && activeBlock === 0 && !typingDone;

              if (block.type === "salutation") {
                return (
                  <p key={i} className="salutation">
                    {block.display}
                    {showSalutationCursor && <span className="cursor">|</span>}
                  </p>
                );
              }
